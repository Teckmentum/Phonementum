...
def validate_gather_id_table(id=None, table_name=None, gather=None):
    """
    Metodo valida que el id pasado sea del formato del primary key para
    la tabla que se intenta conectar en el db. Y que gather sea un digito

    Args:
        id (str): Represent PrimaryKey for the specified table
        table_name (str): the table name from which id is going to be selected
        gather (str): the selected option of the user
    Returns:
        dict: {'isValid': bool, 'error': str, 'status': int}
    Notes:
        1. Author: Glorimar Castro-Noriega
        ...
        3. If isValid is None then an error was rise
    Examples:
        validIdTable("660698757", "company", "1")

    """
    # todo este metodo se debe poder mejorar reutilizando el metodo validate_id_table
    result = {'isValid': None, 'error': None, 'status': None}

    # verify params are not None if param are None return a error: args cannot be None
    if id is None or table_name is None or gather is None:
        result['error'] = db_errors.ArgsCantBeNone("validate_gather_id_table", "id", "table", "gather").message
        result['status'] = 400
    # verify is table_name is valid and if id and gather correspond to a primary key in table_name
    if table_name not in ALLOWED_TABLE_VALUES.keys():
        result['error'] = db_errors.InvalidArgValue(table_name, *ALLOWED_TABLE_VALUES.keys()).message
        result['status'] = 400
    else:
        result['isValid'] = True if (re.match(ALLOWED_TABLE_VALUES[table_name]['id'], id) and re.match(ALLOWED_TABLE_VALUES[table_name]['gather'], gather)) else False
